...
{skill_options}
  ...
  <div class="form-row">
    ...
    <select id="pricing_mode" onchange="onPricingModeChange()" style="min-width:200px">
      <option value="per_ly" selected>Per LY Rate</option>
      <option value="flat">Flat Markup</option>
    </select>
  </div>
  <div class="form-row" id="row_per_ly">
    <label>Rate per LY</label>
    <input type="number" id="fee_per_ly" min="0" step="0.5" value="12" oninput="recalc()" style="width:100px">
    <span class="unit">ISK / m&#179; / LY &nbsp;</span>
    <span class="iv" id="per_ly_note">&mdash;</span>
  </div>
  <div class="form-row" id="row_flat" style="display:none">
...
function onPricingModeChange() {{
  ...
  document.getElementById('row_per_ly').style.display = mode === 'per_ly' ? '' : 'none';
  document.getElementById('row_flat').style.display   = mode === 'flat'   ? '' : 'none';
  recalc();
}}
...
function getMarkupPerM3(ly) {{
  var mode = document.getElementById('pricing_mode').value;
  if (mode === 'per_ly') {{
    var rate = parseFloat(document.getElementById('fee_per_ly').value)||0;
    return rate * ly;
  }} else {{
    return parseFloat(document.getElementById('fee_flat').value)||0;
  }}
}}
...
function recalc() {{
  ...
  var markupPerM3 = getMarkupPerM3(ly);
  ...
  // Per-LY note
  var perLyNoteEl = document.getElementById('per_ly_note');
  if (perLyNoteEl) {{
    var rpl = parseFloat(document.getElementById('fee_per_ly').value)||0;
    perLyNoteEl.textContent = ly > 0 ? fmt(rpl * ly, 2)+' ISK/m\u00B3 on this route' : '\u2014';
  }}

  var configs = getConfigs(adjBase, baseCargo);
  var collFee = coll * collPct / 100;
  var markupFee = markupPerM3 * vol;
  ...
  var results = configs.map(function(c) {{
    ...
    var totalFee  = fuelCost + markupFee + collFee;
    return {{
      econ:          c.econ,
      exp:           c.exp,
      label:         c.label,
