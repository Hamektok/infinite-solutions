"""
generate_standard_ore_margin_report.py
Generates a standalone HTML standard ore margin analysis report for LX-ZOJ.
"""
...
OUT_PATH = os.path.join(os.path.dirname(__file__), 'standard_ore_margin_report.html')

# ── Type ID Constants ────────────────────────────────────────────────────────
STD_COMP = [
    62516, 62517, 62518, 62519,  # Veldspar (base, II, III, IV)
    ...
    62576, 62577, 62578,          # Bezdnacine (null-sec special, no IV)
    62579, 62580, 62581,          # Rakovene
    62582, 62583, 62584,          # Talassonite
...
ALL_PRODUCT_IDS = [pid for pid, _ in MINERALS]
PRODUCT_NAME    = {pid: name for pid, name in MINERALS}

# Ore family → security class
ORE_SEC_CLASS = {
    'Veldspar':    'High-Sec',
    'Scordite':    'High-Sec',
    'Pyroxeres':   'High-Sec',
...
FAMILY_ORDER = [
    ...
    'Jaspet', 'Hemorphite', 'Hedbergite',
    'Gneiss', 'Dark Ochre', 'Crokite', 'Bistot', 'Arkonor', 'Spodumain', 'Mercoxit',
    'Bezdnacine', 'Rakovene', 'Talassonite',
]

SEC_ORDER = {'High-Sec': 0, 'Low-Sec': 1, 'Null-Sec': 2, 'Special': 3}
SEC_COLORS = {
    'High-Sec': '#4a9eff',
    'Low-Sec':  '#34d058',
    'Null-Sec': '#f4a700',
...
def ore_family(name: str) -> str:
    """Extract ore family name by stripping 'Compressed ' and grade suffixes."""
...
def generate_ore_narrative(family: str, base_margin: float | None,
    ...
    sec_class = ORE_SEC_CLASS.get(family, 'Unknown')
    s1 = (f"{family} ({sec_class}) currently offers a {margin_desc} margin of "
...
def generate_exec_summary(analyses: list) -> dict:
    ...
    # Count profitable by sec class
    by_sec = {}
    for a in valid:
        sc = a['sec_class']
        if sc not in by_sec:
            by_sec[sc] = []
        by_sec[sc].append(a['current_margin'])

    para_parts = [
        f"Standard ore margins are {trend_word} — "
        f"{improving} ores trending up, {declining} down, {mixed} flat over 30 days."
    ]

    # Best per sec class
    for sc in ['High-Sec', 'Low-Sec', 'Null-Sec', 'Special']:
        margs = by_sec.get(sc, [])
        if margs:
            avg = sum(margs) / len(margs)
            para_parts.append(f"{sc} avg: {avg:.1f}% ROI.")
...
def load_data():
    ...
    prod_pcts = {}
    for pid, _ in MINERALS:
        key = f'ore_pct_{pid}'
        prod_pcts[pid] = float(cfg.get(key, '95')) / 100.0

    all_ids = STD_COMP + ALL_PRODUCT_IDS
    ph = ','.join('?' * len(all_ids))
    cur.execute(
...
def sec_badge_class(sec_class):
    return {
        'High-Sec': 'badge-highsec',
        'Low-Sec':  'badge-lowsec',
        'Null-Sec': 'badge-nullsec',
...
def build_html(analyses, exec_summary, prod_jbv_now, prod_jbv_7d, prod_jbv_30d,
    ...
    def family_sort_key(fam):
        sc = ORE_SEC_CLASS.get(fam, 'Unknown')
        sci = SEC_ORDER.get(sc, 99)
        try:
    ...
    for fam in sorted_families:
        ...
        sec_class   = base_ore['sec_class']
        sbc         = sec_badge_class(sec_class)
...
"""
        ...
        <div class="family-card" data-sec="{sec_class}">
            <div class="family-header">
                <span class="family-name">{fam}</span>
                <span class="family-sec {sbc}">{sec_class}</span>
...
""")
    ...
    # Section 4 — Mineral Market Overview
    prod_rows_s4 = ''
    prod_jbv_changes = []
    for pid, pname in MINERALS:
    ...
    if prod_jbv_changes:
        ...
        if avg_chg > 3:
            prod_summary = f"Minerals are trending upward on average ({avg_chg:+.1f}% over 30 days), which improves refining returns across all ore classes."
        elif avg_chg < -3:
            prod_summary = f"Minerals are under selling pressure ({avg_chg:+.1f}% over 30 days), compressing margins across the board."
        else:
            prod_summary = f"Mineral prices have been broadly stable over the past 30 days (avg {avg_chg:+.1f}%), providing predictable refining revenue."
    else:
        prod_summary = "Insufficient data for mineral trend summary."
...
"""
    ...
    for a in sorted_analyses:
        ...
        sig = a['signal']
        sc  = signal_class(sig)
        sbc = sec_badge_class(a['sec_class'])
        ranking_rows += (
            f"<tr class='{rc}' data-sec='{a['sec_class']}'>"
            f"<td>{a['name']}</td>"
            f"<td><span class='{sbc}'>{a['sec_class']}</span></td>"
...
<title>Standard Ore Margin Report — LX-ZOJ</title>
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  /* Sec class badges */
  .badge-highsec {{ color: var(--highsec); font-weight: 700; }}
  .badge-lowsec  {{ color: var(--lowsec);  font-weight: 700; }}
  .badge-nullsec {{ color: var(--nullsec); font-weight: 700; }}
  .badge-special {{ color: var(--special); font-weight: 700; }}
  ...
  .family-sec  {{ font-size: 12px; font-weight: 700; }}
...
<div class="report-header">
  <div class="container" style="padding-bottom:0;">
    <h1>Standard Ore Margin Report — LX-ZOJ</h1>
...
<div class="container">
  ...
  <div class="filter-bar" id="rankings-filter-bar">
    <button class="filter-btn active" onclick="filterRankings('all', this)">All</button>
    <button class="filter-btn badge-highsec" onclick="filterRankings('High-Sec', this)">High-Sec</button>
    <button class="filter-btn badge-lowsec"  onclick="filterRankings('Low-Sec',  this)">Low-Sec</button>
    <button class="filter-btn badge-nullsec" onclick="filterRankings('Null-Sec', this)">Null-Sec</button>
    <button class="filter-btn badge-special" onclick="filterRankings('Special',  this)">Special</button>
  </div>
  ...
  <div class="filter-bar" id="family-filter-bar">
    <button class="filter-btn active" onclick="filterFamilies('all', this)">All</button>
    <button class="filter-btn badge-highsec" onclick="filterFamilies('High-Sec', this)">High-Sec</button>
    <button class="filter-btn badge-lowsec"  onclick="filterFamilies('Low-Sec',  this)">Low-Sec</button>
    <button class="filter-btn badge-nullsec" onclick="filterFamilies('Null-Sec', this)">Null-Sec</button>
    <button class="filter-btn badge-special" onclick="filterFamilies('Special',  this)">Special</button>
  </div>
  ...
  <!-- Section 4: Mineral Market Overview -->
  <h2 class="section-title">Mineral Market Overview</h2>
  {sec4_html}
...
function filterRankings(sec, btn) {{
  document.querySelectorAll('#rankings-filter-bar .filter-btn').forEach(function(b) {{
    b.classList.remove('active');
  }});
  btn.classList.add('active');
  document.querySelectorAll('#rankings-tbody tr').forEach(function(r) {{
    if (sec === 'all' || r.dataset.sec === sec) {{
      r.style.display = '';
    }} else {{
...
// Filter: Family Cards
function filterFamilies(sec, btn) {{
  ...
  document.querySelectorAll('#family-cards-container .family-card').forEach(function(card) {{
    if (sec === 'all' || card.dataset.sec === sec) {{
      card.style.display = '';
    }} else {{
