...
{systems_datalist}
  ...
  <div class="form-row">
    ...
    <input type="text" id="dest_input" placeholder="Auto-filled&hellip;"
      oninput="recalc()" autocomplete="off" disabled
...
function onPickupChange() {{
  ...
  if (pickup === HUB) {{
    // Outbound: enable dest field so customer can pick freely
    ...
    // No pickup: keep dest disabled and blank
    destEl.disabled = true;
    destEl.removeAttribute('list');
    destEl.value = '';
    destEl.style.opacity = '0.6';
    destEl.placeholder = 'Auto-filled\u2026';
    distEl.textContent = '';
  }} else {{
    // Inbound: disable dest entirely, set value to HUB
    destEl.disabled = true;
    destEl.removeAttribute('list');
    destEl.value = HUB;
    destEl.style.opacity = '0.6';
